...
def render_flashcard(flashcard: Dict, on_update: Callable[[int, str], None], on_delete: Callable[[int], None]):
    """
    ...
    with st.container():
        ...
        with col1:
            ...
            if flashcard.get("last_checked_at"):
                last_checked_dt = datetime.fromisoformat(flashcard["last_checked_at"].replace("Z", "+00:00"))
        ...
        with col2:
            ...
            if current_status == "to-learn":
                st.button("Mark as Checked", key=f"once_{flashcard['id']}", 
                ...
                if flashcard.get("status_once_checked_at"):
                    checked_at = datetime.fromisoformat(flashcard["status_once_checked_at"].replace("Z", "+00:00"))
                    time_passed = datetime.now(timezone.utc) - checked_at
                    
                    if time_passed.total_seconds() >= 24 * 3600:
                        st.button("Mark as Twice Checked", key=f"twice_{flashcard['id']}", 
                                  on_click=lambda: on_update(flashcard["id"], "twice-checked"))
                    else:
                ...
                if flashcard.get("status_twice_checked_at"):
                    checked_at = datetime.fromisoformat(flashcard["status_twice_checked_at"].replace("Z", "+00:00"))
                    time_passed = datetime.now(timezone.utc) - checked_at
                    
                    if time_passed.total_seconds() >= 48 * 3600:
                        st.button("Mark as Memorized", key=f"memo_{flashcard['id']}", 
                                  on_click=lambda: on_update(flashcard["id"], "fully-memorized"))
                    else:
                        hours_left = 48 - (time_passed.total_seconds() / 3600)
                        st.write(f"⏳ {hours_left:.1f} hours left")
            
            # Delete button for all cards
            st.button("🗑️", key=f"del_{flashcard['id']}", 
...
def render_add_flashcard_form(user_id: str, on_add: Callable[[], None]):
    ...
    with st.form("add_flashcard", clear_on_submit=True):
        st.subheader("Add New German Vocabulary/Phrase")
        content = st.text_area("Enter vocabulary, sentence, or phrase to learn:", 
...
def render_flashcard_stats(flashcards: List[Dict]):
    ...
    fully_memorized = sum(1 for card in flashcards if card["status"] == "fully-memorized")
    
    # Create a progress bar for overall progress
    progress = (once_checked * 0.33 + twice_checked * 0.67 + fully_memorized) / total if total > 0 else 0
